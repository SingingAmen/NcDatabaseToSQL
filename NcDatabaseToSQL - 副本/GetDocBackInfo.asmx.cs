    ...
    public class GetDocBackInfo : System.Web.Services.WebService
    {
        ...
        public string GetDocBackMemoInfo()
        {
            ...
            try
            {
                ...
                //获取满足条件的单号
                sql = "select '形态转换' docType,code,memo,SUBSTRING(ddate,0,8) ddate,GETDATE() createtime from AssemVouch where zt=0 union all select '销售出库' docType,code,memo,SUBSTRING(ddate,0,8) ddate,GETDATE() createtime from DispatchList where zt=0 union all select '采购发票' docType,code,memo,SUBSTRING(ddate,0,8) ddate,GETDATE() createtime from PurBillVouch where zt=0 union all select '采购入库' docType,code,memo,SUBSTRING(ddate,0,8) ddate,GETDATE() createtime from Rdrecord01 where zt=0 union all select '其他入库' docType,code,memo,SUBSTRING(ddate,0,8) ddate,GETDATE() createtime from RdRecord08 where zt=0 union all select '其他出库' docType,code,memo,SUBSTRING(ddate,0,8) ddate,GETDATE() createtime from RdRecord09 where zt=0 union all select '产成品入库' docType,code,memo,SUBSTRING(ddate,0,8) ddate,GETDATE() createtime from RdRecord10 where zt=0 union all select '材料出库' docType,code,memo,SUBSTRING(ddate,0,8) ddate,GETDATE() createtime from RdRecord11 where zt=0 union all select '销售发票' docType,code,memo,SUBSTRING(ddate,0,8) ddate,GETDATE() createtime from SaleBillVouch where zt=0 union all select '调拨单'  docType,code,memo,SUBSTRING(ddate,0,8) ddate,GETDATE() createtime from TransVouch where zt=0";
