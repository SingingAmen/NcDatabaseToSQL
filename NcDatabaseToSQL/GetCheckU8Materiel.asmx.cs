        ...
        public string GetDocNOFromNc(string datetime)
        {
            ...
            try
            {
                ...
                if (existResult == 0)
                {
                    ...
                    //获取满足条件的单号
                    sql = "select distinct A.vbillcode DocNO,'材料出库' DocName,A4.code ItemCode,GETDATE() as CreateTime from ic_material_h A left join ic_material_b A1 on A.cgeneralhid = A1.cgeneralhid left join bd_billtype A2 on A2.pk_billtypeid=A.ctrantypeid left join org_dept_v A3 on A3.pk_vid=A.cdptvid left join bd_material A4 on A1.cmaterialvid = A4.pk_material where A2.billtypename='生产辅助领用（名鸿）' and A3.name not like '%行政%' and substr(A4.code,1,2) not in('01','03','10','11','12','13','14')";
                }
                ...
                if (existResult == 0)
                {
                    createSql = "create table Spl_CheckNCItemCode(Id [bigint] primary key not null,[DocNO] [nvarchar](50) NULL,[DocName] [nvarchar](200) NULL,[ItemCode] [nvarchar](200) NULL,[ItemName] [nvarchar](200) NULL,[def1] [nvarchar](200) NULL,[def2] [nvarchar](200) NULL,[def3] [nvarchar](200) NULL,[def4] [nvarchar](200) NULL,[def5] [nvarchar](200) NULL,[def6] [nvarchar](200) NULL,[def7] [nvarchar](200) NULL,[def8] [nvarchar](200) NULL,[def9] [nvarchar](200) NULL,[def10] [nvarchar](200) NULL,[CreateTime] [datetime] NOT NULL)";
