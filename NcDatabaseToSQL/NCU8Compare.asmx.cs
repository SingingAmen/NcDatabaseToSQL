...
namespace NcDatabaseToSQL
{
    ...
    public class NCU8Compare : System.Web.Services.WebService
    {
        ...
        private string GetGeneraloutToSql()
        {
            ...
            {
                //获取其他出库单数据
                sql = "select '其他出库单' cbustype,A1.cInvCode cInvCode,SUM(A1.iQuantity) iQuantity,A.cWhCode publicsec1,A2.cWhName publicsec2,GETDATE() as CreateTime from RdRecord09 A left join rdrecords09 A1 on A.ID=A1.ID left join Warehouse A2 on A2.cWhCode=A.cWhCode WHERE Convert(nvarchar(7),A.dDate,121)='" + queryDate + "' and A.cRdCode NOT IN ('207','210') group by A1.cInvCode,A.cWhCode,A2.cWhName";
        ...
        private string GetGeneralinToSql()
        {
            ...
            {
                //获取其他出库单数据
                sql = "select '其他入库单' cbustype,A1.cInvCode cInvCode,SUM(A1.iQuantity) iQuantity,A.cWhCode publicsec1,A2.cWhName publicsec2,GETDATE() as CreateTime from RdRecord08 A left join rdrecords08 A1 on A.ID=A1.ID left join Warehouse A2 on A2.cWhCode=A.cWhCode WHERE Convert(nvarchar(7),A.dDate,121)='" + queryDate + "' and A.cRdCode NOT IN ('107','105') group by A1.cInvCode,A.cWhCode,A2.cWhName";
        ...
        private string GetTransformToSql()
        {
            ...
            {
                //获取其他出库单数据
                sql = "select '形态转换单' cbustype,A1.cInvCode cInvCode,SUM(A1.iAVQuantity) iQuantity,A1.cWhCode publicsec1,A2.cWhName publicsec2,GETDATE() as CreateTime from AssemVouch A left join AssemVouchs A1 on A.ID=A1.ID left join Warehouse A2 on A2.cWhCode=A.cWhCode WHERE Convert(nvarchar(7),A.dAVDate,121)='" + queryDate + "' group by A1.cInvCode,A1.cWhCode,A2.cWhName";
        ...
        private string Ncu8Compare()
        {
            ...
            try
            {
                ...
                SqlHelperForApps.ExecuteNonQuerys(delstrApps);
                sql = " select isnull(A.cbustype,A1.cbustype) cbustype,isnull(A.cinvcode,A1.cinvcode) cInvCode,isnull(A.iquantity,0) iQuantity,isnull(A1.iquantity,0) nciQuantity,isnull(A.publicsec1,A1.publicsec1) publicsec1,isnull(A.publicsec2,A1.publicsec2) publicsec2,isnull(A.publicsec3,A1.publicsec3) publicsec3,isnull(A.publicsec4,A1.publicsec4) publicsec4,GETDATE() as CreateTime from [Spl_U8DBData] A full join [Spl_ncDBData] A1 on A.cbustype=A1.cbustype and A.cinvcode = A1.cinvcode and  A.publicsec1=A1.publicsec1 and  A.iquantity=A1.iquantity where A.cbustype!='采购发票' and A1.cbustype!='采购发票' and A.cbustype!='销售发票' and A1.cbustype!='销售发票' order by CreateTime asc";
        ...
        private string Ncu8CompareForCGFP()
        {
            ...
            try
            {
                ...
                SqlHelperForApps.ExecuteNonQuerys(delstrApps);
                string sql2 = " select isnull(A.cbustype,A1.cbustype) cbustype,isnull(A.cinvcode,A1.cinvcode) cInvCode,isnull(A.iquantity,0) iQuantity,isnull(A1.iquantity,0) nciQuantity,isnull(A.publicsec1,A1.publicsec1) publicsec1,isnull(A.publicsec2,0) publicsec2,isnull(A1.publicsec2,0) publicsec5,isnull(A.publicsec3,A1.publicsec3) publicsec3,isnull(A.publicsec4,A1.publicsec4) publicsec4,GETDATE() as CreateTime from [Spl_U8DBData] A full join [Spl_ncDBData] A1 on A.cbustype=A1.cbustype and A.cinvcode = A1.cinvcode and  A.publicsec4=A1.publicsec4 and  A.publicsec1=A1.publicsec1 where A.cbustype='采购发票' or A1.cbustype='采购发票' order by A.publicsec4 desc";
                //获取采购入库汇总数据
        ...
        private string Ncu8CompareForXSFP()
        {
            ...
            try
            {
                ...
                SqlHelperForApps.ExecuteNonQuerys(delstrApps);
                string sql2 = " select isnull(A.cbustype,A1.cbustype) cbustype,isnull(A.cinvcode,A1.cinvcode) cInvCode,isnull(A.iquantity,0) iQuantity,isnull(A1.iquantity,0) nciQuantity,isnull(A.publicsec1,A1.publicsec1) publicsec1,isnull(A.publicsec2,0) publicsec2,isnull(A1.publicsec2,0) publicsec5,isnull(A.publicsec3,A1.publicsec3) publicsec3,isnull(A.publicsec4,A1.publicsec4) publicsec4,GETDATE() as CreateTime from [Spl_U8DBData] A full join [Spl_ncDBData] A1 on A.cbustype=A1.cbustype and A.cinvcode = A1.cinvcode and  A.publicsec4=A1.publicsec4 and  A.publicsec1=A1.publicsec1 where  A.cbustype='销售发票' or A1.cbustype='销售发票' order by A.publicsec4 desc";
                //获取采购入库汇总数据
                DataSet ncu8compareDs = SqlHelper.ExecuteDataset(connectionString, CommandType.Text, sql2);
                StringBuilder strt = DataSetToArrayList.DataSetToArrayLists(ncu8compareDs, "Spl_NCU8Compare");
                SqlHelperForApps.ExecuteNonQuery(strt.ToString());
