...
namespace NcDatabaseToSQL
{
    ...
    public class NcDatabaseToSQL : System.Web.Services.WebService
    {
        ...
        {
            string msg = "";
            NcDatabaseToSQLForBulk ncDatabaseToSQLForBulk = new NcDatabaseToSQLForBulk();
            switch (interfaceParameters)
            {
                //物料清单
                case "wlqd":
                ...
                case "ywdjdrbluk":
                    msg = ncDatabaseToSQLForBulk.NcDatabaseToSQL();
                    break;
                case "ywdjdr":
                    //采购入库
                    msg = GetPurchaseinToSql() + "/";
        ...
        private string GetBomToSql()
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    string dellll = "delete from Boms where ID not in(select id from Bom)";
                    string delstr3 = "delete from BomNew";//更新表
                    SqlHelper.ExecuteNonQuerys(dellll);
                    SqlHelper.ExecuteNonQuerys(delstr2);
                    SqlHelper.ExecuteNonQuerys(delstr);
                    ...
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        ...
                        //获取bom头
                        sql = "select distinct A.cbomid ID,A2.code cinvcode,A2.name cinvname, A3.code cinvclass,A3.name cinvclassname,A2.materialspec cinvstd, A4.code cinvUnit,A.ts ddate, A.hversion bomversion, A.Hvdef1 bomversionexplain, A.DR dr, A.modifiedtime ts from bd_bom A left join bd_bom_b A1 on A.cbomid = A1.cbomid left join bd_material A2 on A2.pk_material = A.hcmaterialid left join bd_marbasclass A3 ON A2.PK_MARBASCLASS = A3.PK_MARBASCLASS left join bd_measdoc A4 on A2.PK_MEASDOC = A4.pk_measdoc where A.PK_ORG = '0001A110000000001V70' and A.hversion not in('1.0', '1.1') and substr(A2.code,1,2) != 13 AND (substr(A.creationtime,0,10) >= '2021-01-01'  or A.ts>= '2021-01-01') and " + strGetOracleSQLIn + "";
                    }
                    else
                    {
                        updateCount = 0;
        ...
        /// 从nc获取仓库信息插入到sql
        /// 创建人：lvhe
        /// 创建时间：2021-01-20 18:28:59
        /// </summary>
        /// <returns></returns>
        private string GetWhToSql()
        {
            string result = "";
            string createSql = "";
            string tableExist = "";
            ...
            {
                //判断当前表是否存在 1存在 0 不存在
                tableExist = "if object_id( 'CBO_WH') is not null select 1 else select 0";
                existResult = SqlHelper.ExecuteNonQuerys(tableExist);
                if (existResult == 0)
                {
                    //获取采购入库头数据
                    sql = "select code,name,code addr,case when csflag='N' then 'false' else 'true' end  bwhpos,creationtime  ddate from bd_stordoc where dr!=1 and PK_ORG='0001A110000000001V70'";
                }
                else
                {
                    string delstr = "delete from CBO_WH";
                    SqlHelper.ExecuteNonQuerys(delstr);
                    sql = "select code,name,code addr,case when csflag='N' then 'false' else 'true' end  bwhpos,creationtime  ddate from bd_stordoc where dr!=1 and PK_ORG='0001A110000000001V70'";
                }
                //获取采购入库头数据
                DataSet WhData = OracleHelper.ExecuteDataset(sql);
                ...
                if (existResult == 0)
                {
                    createSql = "create table CBO_WH(code nvarchar(50),name nvarchar(100),addr nvarchar(500), bwhpos bit,ddate datetime,zt bit default 0,memo text)";
                    SqlHelper.ExecuteNonQuery(createSql);
                    StringBuilder str = DataSetToArrayList.DataSetToArrayLists(WhData, "CBO_WH");
                    SqlHelper.ExecuteNonQuery(str.ToString());
                    msg = "仓库表插入成功";
                }
                else
                ...
                GetU8SVApiUrlApi("ckxxapi");
                result = msg;
            }
            catch (Exception e)
            {

                result = "仓库表错误：" + e.Message;
            }
            return result;
        }
        ...
        private string GetInventoryToSql()
        {
            ...
            try
            {
                ...
                if (existResult == 0)
                {
                    ...
                    if (JudgeDs(Inventory))
                    {
                        SqlBulkCopyHelperForTest.ImportTempTableDataIndex(Inventory, "CBO_Inventory");
                        msg = "物料档案表插入成功";
                    }
                }
                else
